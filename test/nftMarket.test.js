...
contract("NftMarket", (accounts) => {
  ...
  describe("Mint token", () => {
    ...
    it("first token should point to the corrent tokenURI", async () => {
      const actualTokenURI = await _contract.tokenURI(1)

      assert.equal(actualTokenURI, tokenURI, "tokenURI is not correctly set")
    })

    it("should not be possible to create a NFT with used token", async () => {
      try {
        await _contract.mintToken(tokenURI, _nftPrice, {
          from: accounts[0]
        })    
    ...
    it("should have on listed item", async () => {
  ...
  describe("Buy NFT", () => {
    ...
    it("should change the owner", async () => {
      ...
      assert.equal(currentOwner, accounts[1], "Owner is not change")
    })
  })
  ...
  describe("Token transfers", () => {
    ...
    it("should have on listed NFT", async () => {
      const allNfts = await _contract.getAllNftOnSale();

      assert.equal(allNfts[0].tokenId, 2, "Nft wrong id")
    })
  ...
  describe("List an Nft", () => {
    before(async () => {
      await _contract.placeNftOnSale(
        1, _nftPrice, {from: accounts[1], value: _listingPrice}
      )
    })

    it("should have two listed items", async () => {
      const listedNft = await _contract.getAllNftOnSale()
      assert.equal(listedNft.length, 2, "Invalid length of nft")
    })
    ...
    it("should set new listing price", async () => {
      ...
      assert.equal(listingPrice.toString(), _listingPrice.toString(), "Invalid price")
    })
  })
